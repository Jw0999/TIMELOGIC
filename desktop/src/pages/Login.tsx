...
export default function Login() {
  ...
  const [identifier, setIdentifier] = useState('');   // accepts email OR admin code
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (!identifier.trim()) { setError('Please enter your email or admin code.'); return; }
    ...
    if (ok) {
      ...
      setError('Invalid credentials. Check your email/code and password, or contact your Super Admin.');
    }
  };
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-3xl p-8 shadow-2xl">
          ...
          <p className="text-sm text-slate-500 mb-6">
            Sign in with your admin email or admin code provided by the Super Admin.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1.5">
                Email Address or Admin Code
              </label>
              <div className="relative">
                {isEmail
                  ? <Mail size={17} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                  : <IdCard size={17} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                }
                <input
                  type="text"
                  value={identifier}
